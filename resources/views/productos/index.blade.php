    ...
    <div class="grid grid-cols-2 lg:grid-cols-6 gap-4 sm:gap-6">
        <div class="bg-app-card rounded-2xl shadow-lg border border-app-accent/50 p-5 hover:border-blue-500/50 transition-colors group">
            ...
            <div class="flex items-end justify-between">
                <p class="text-2xl sm:text-3xl font-bold text-white group-hover:text-blue-400 transition-colors">{{ $totalProductos }}</p>
        ...
        <div class="bg-app-card rounded-2xl shadow-lg border border-app-accent/50 p-5 hover:border-emerald-500/50 transition-colors group">
            ...
            <div class="flex items-end justify-between">
                <p class="text-2xl sm:text-3xl font-bold text-white group-hover:text-emerald-400 transition-colors">{{ $totalActivos }}</p>
                <div class="text-emerald-400/50 group-hover:text-emerald-400 transition-colors">
                    <svg class="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
            </div>
        </div>
        
        <div class="bg-app-card rounded-2xl shadow-lg border border-app-accent/50 p-5 hover:border-slate-500/50 transition-colors group">
            <h3 class="text-app-textMuted text-xs font-semibold uppercase tracking-wider mb-2">Inactivos</h3>
            <div class="flex items-end justify-between">
                <p class="text-2xl sm:text-3xl font-bold text-white group-hover:text-slate-400 transition-colors">{{ $totalInactivos }}</p>
                <div class="text-slate-400/50 group-hover:text-slate-400 transition-colors">
                    <svg class="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path></svg>
                </div>
            </div>
        </div>
        
        <div class="bg-app-card rounded-2xl shadow-lg border border-app-accent/50 p-5 hover:border-amber-500/50 transition-colors group col-span-2 lg:col-span-1">
            <h3 class="text-app-textMuted text-xs font-semibold uppercase tracking-wider mb-2">Valor Inventario</h3>
            <div class="flex items-end justify-between">
                <p class="text-xl sm:text-2xl font-bold text-white group-hover:text-amber-400 transition-colors">${{ number_format($valorInventario, 2) }}</p>
        ...
        <div class="relative group bg-app-card rounded-2xl shadow-lg border border-app-accent/50 hover:border-red-500/50 p-5 transition-colors cursor-help col-span-1 lg:col-span-1"
            ...
            <div id="modal-hover-stock" class="absolute top-full right-0 mt-3 w-64 bg-app-card border border-app-accent shadow-2xl rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 p-4">
                <h4 class="text-xs font-bold text-red-400 uppercase mb-3 flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span> Productos Críticos</h4>
        ...
        <div class="relative group bg-app-card rounded-2xl shadow-lg border border-app-accent/50 hover:border-indigo-500/50 p-5 transition-colors cursor-help col-span-1 lg:col-span-1"
            ...
            <div class="flex items-end justify-between">
                <p class="text-2xl sm:text-3xl font-bold text-white group-hover:text-indigo-400 transition-colors">{{ $totalCategorias }}</p>
            ...
            <div id="modal-hover-categorias" class="absolute top-full right-0 mt-3 w-56 bg-app-card border border-app-accent shadow-2xl rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 p-4">
                <h4 class="text-xs font-bold text-indigo-400 uppercase mb-3 border-b border-app-accent/50 pb-2">En Uso</h4>
    ...
    <div class="bg-app-card rounded-2xl shadow-lg border border-app-accent/50 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-app-bg/80 border-b border-app-accent/50 text-app-textMuted tracking-wider uppercase text-[10px] sm:text-xs">
                ...
                <tbody id="productos-list" class="divide-y divide-app-accent/30 text-sm">
                    ...
                    <tr class="hover:bg-app-bg/50 transition-colors group" id="prod-{{ $producto->id }}">
                        <td class="px-6 py-4">
                            <div class="font-semibold text-white group-hover:text-app-primary transition-colors">{{ $producto->nombre }}</div>
                            @if($producto->codigo_barras)
                            <div class="text-xs text-app-textMuted/70 mt-1 font-mono bg-app-bg inline-block px-2 py-0.5 rounded">{{ $producto->codigo_barras }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <span class="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xs px-2.5 py-1 rounded-lg">{{ $producto->categoria }}</span>
                        </td>
                        <td class="px-6 py-4 font-medium text-white">${{ number_format($producto->precio, 2) }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <span class="font-bold {{ $producto->stock <= 5 ? 'text-red-400 bg-red-400/10 px-2 py-0.5 rounded' : 'text-emerald-400' }}">
                                    {{ $producto->stock }}
                                </span>
                            </div>
                        </td>
...
{{-- Modal --}}
<div id="producto-modal" class="fixed inset-0 bg-app-bg/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 hidden opacity-0 transition-opacity duration-300">
    <div class="bg-app-card rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-app-accent w-full max-w-2xl max-h-[90vh] flex flex-col transform scale-95 transition-transform duration-300" id="modal-content">
        
        <div class="p-6 border-b border-app-accent/50 flex justify-between items-center bg-app-card rounded-t-2xl z-10 sticky top-0">
            ...
            <button onclick="cerrarModal()" class="text-app-textMuted hover:text-white transition-colors p-1 bg-app-bg rounded-lg border border-app-accent hover:bg-red-500/20 hover:border-red-500/50">
        ...
        <div class="p-6 overflow-y-auto custom-scrollbar">
            ...
            <form id="producto-form" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Nombre *</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Precio Venta *</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Precio de Compra</label>
                        <div class="relative">
                            <span class="absolute left-4 top-3.5 text-app-textMuted">$</span>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Stock Actual</label>
                        <input type="number" min="0" id="f-stock"
                            class="w-full px-4 py-3 bg-app-bg border border-app-accent rounded-xl text-white focus:outline-none focus:border-app-primary focus:ring-1 focus:ring-app-primary transition"
                            placeholder="0">
                    </div>
                    
                    <div id="suma-stock-container" class="hidden relative p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
                        <label class="block text-xs font-semibold text-emerald-500 uppercase tracking-wider mb-2">Sumar Stock (+)</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Categoría</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Código de Barras</label>
                        <div class="flex gap-2 mb-2">
                            ...
                            <button type="button" onclick="iniciarEscaner()" id="btn-escaner" class="px-4 py-3 bg-app-bg border border-app-accent rounded-xl text-app-textMuted hover:text-white hover:border-app-primary transition-colors flex items-center gap-2 tooltip" title="Escanear con Cámara">
                                📷 <span class="hidden sm:inline text-xs">Escanear</span>
                            </button>
                        </div>
                        <div id="reader-container" class="hidden w-full overflow-hidden rounded-xl border border-app-primary/50 relative bg-black">
                            <button type="button" onclick="detenerEscaner()" class="absolute top-2 right-2 z-10 bg-red-500 text-white p-1 rounded-lg text-xs font-bold hover:bg-red-600">Cerrar</button>
                            <div id="reader" class="w-full"></div>
                        </div>
                    </div>
                    
                    <div class="md:col-span-2">
                        <label class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Descripción</label>
...
function abrirModal() {
    ...
    document.getElementById('f-stock').classList.remove('opacity-50', 'bg-app-bg');
...
function mostrarToast(msg, tipo) {
    ...
    div.className = `fixed bottom-6 right-6 transform transition-all duration-300 translate-y-10 opacity-0 px-6 py-4 rounded-xl shadow-2xl z-[100] border font-medium flex items-center gap-3 ${isExito ? 'bg-emerald-500 text-white border-emerald-400' : 'bg-red-500 text-white border-red-400'}`;
...
async function mostrarDetallesStock() {
    ...
    try {
        ...
        if (productos.length === 0) {
            lista.innerHTML = '<li class="text-emerald-500 font-medium bg-emerald-500/10 p-2 rounded-lg text-center">Todo en orden ✅</li>';
        } else {

...
<div class="space-y-8 animate-fade-in relative">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-6 gap-4 sm:gap-6">
        ...
        <div
            class="bg-app-card rounded-2xl shadow-lg border border-app-accent/50 p-5 hover:border-amber-500/50 transition-colors group col-span-2 lg:col-span-1 overflow-hidden">
            <h3 class="text-app-textMuted text-[10px] uppercase font-semibold tracking-wider mb-3 truncate">
                Valor Inventario
            </h3>
            ...
            <div class="grid grid-cols-[1fr_auto] gap-2 items-start">
                ...
                <div class="flex flex-col min-w-0">
                    <p
                        class="text-xl sm:text-2xl font-bold text-white group-hover:text-amber-400 transition-colors leading-none truncate">
        ...
        : ($valorInventario >= 1000 ? number_format($valorInventario / 1000, 1) . 'K' : number_format($valorInventario, 2)) 
                    ...
                    <p
                        class="text-[10px] text-app-textMuted font-mono mt-2 opacity-80 whitespace-nowrap overflow-hidden text-ellipsis">
        ...
        <div class="p-6 overflow-y-auto custom-scrollbar">
            ...
            <form id="producto-form" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label
                            class="block text-xs font-semibold text-app-textMuted uppercase tracking-wider mb-2">Proveedor</label>
                        <select id="f-proveedor_id"
                            class="w-full px-4 py-3 bg-app-bg border border-app-accent rounded-xl text-white focus:outline-none focus:border-app-primary focus:ring-1 focus:ring-app-primary transition outline-none appearance-none">
                            <option value="">Sin proveedor asociado</option>
                            @foreach($proveedores as $prov)
                            <option value="{{ $prov->id }}">
                                {{ $prov->nombre }}@if($prov->empresa) ({{ $prov->empresa }})@endif -
                                {{ $prov->link_web ?? 'Indefinido' }}
                            </option>
                            @endforeach
                        </select>
                    </div>
    ...
    filterTimeout = setTimeout(async () => {
        ...
        try {
            ...
            tbody.innerHTML = productos.map(p => {
                ...
                const catClass = catColorsJS[p.categoria] ||
